...
if [ "$1" == "-p" ]
   then
     ...
     echo -e " Encrypted Password below : "
        echo $varpassword1 | openssl enc -aes-256-cbc -salt -a -pass pass:$( df -hT / | tail -1 | awk '{print $2}' )
     echo -e " "
     exit
fi
...
echo -e "                  <line x1='381' y1='21' x2='381' y2='15' style='stroke:rgb(0,0,0);stroke-width:2'></line> \
   ...
   echo -e "                  <line x1='23' y1='$cpuperline' x2='381' y2='$cpuperline' style='stroke:rgb(240,240,240);stroke-width:1'></line>" \
        ...
        # Get COD or TIER CPU performance limit.
        # SED her will grep out numbers between the brackets ().
/opt/teradata/tdat/pde/$(/usr/pde/bin/pdepath -i | grep PDE: | cut -d' ' -f2)/bin/pdeglobal tpa \
    | grep psfcpufactor | awk '{ print $2 }' | sed -e 's/.*(\(.*\)).*/\1/' | cut -d '.' -f1 > $dumpfolder\cpulimit.info.actual.txt

cpuperfact=$(cat $dumpfolder\cpulimit.info.actual.txt)
if [ $cpuperfact -lt 100 ]
    then
cpulimitinfo=$(( $(cat $dumpfolder\cpulimit.info.actual.txt) + 20 ))
   echo -e "                  <line x1='23' y1='$cpulimitinfo' x2='381' y2='$cpulimitinfo' style='stroke:rgb(255,0,0);stroke-width:1'></line>" \
         >> $dumpfile
fi  &&
...
cat $tempfolder/ALL.Nodes.sensor.data.txt \
     | sed -n "/$bynname/,/^$/p" \
     | egrep 'Fan[0-9]|Fan [0-9]|Temp|Current|Voltage|PS|Mem ECC|Mem CRC|Mem Fatal|Pwr Consumption' \
...
.logon /$btequser,$( echo $cactus1 | openssl enc -aes-256-cbc -salt -a -d -pass pass:$( df -hT / | tail -1 | awk '{print $2}' ));
